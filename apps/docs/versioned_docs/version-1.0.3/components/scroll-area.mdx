...
Scroll areas provide a highly customizable scroll container with dynamic fade masks, macOS-style hover expansions, custom scroll buttons, and Framer Motion powered entrance animations.
...
| `animation` | `"fade" \| "slide" \| "scale" \| "none"` | `"none"` | Entrance animation for the internal content. |
| `showProgress` | `boolean` | `false` | Shows a progress bar at the top of the container to indicate scroll depth. |
| `showScrollButtons` | `boolean` | `false` | Renders functional floating chevron buttons allowing users to scroll to edges directly. |
| `expandOnHover` | `boolean` | `true` | If true, the scrollbar expands its thickness when hovered over. |
